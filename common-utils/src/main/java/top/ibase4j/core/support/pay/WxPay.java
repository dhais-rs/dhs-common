...
public class WxPay {
     ...
     * @return
     */
    public static String getCodeUrl(String appid, String mchId, String productId, String partnerKey,
        ...
        String qrCodeUrl = WxPayment.replace(url, "XXXXX", packageSign, appid, mchId, productId, timeStamp, nonceStr);
        if (isToShortUrl) {
            String shortResult = WxPay
                .toShortUrl(WxPayment.buildShortUrlParasMap(appid, null, mchId, null, qrCodeUrl, partnerKey));
            logger.info(shortResult);
            Map<String, String> shortMap = WxPayment.xmlToMap(shortResult);
            String returnCode = shortMap.get("return_code");
            if (WxPayment.codeIsOK(returnCode)) {
                String resultCode = shortMap.get("result_code");
