        ...
        try {
            if (o instanceof ArrayList<?>) {
                ...
                if (o == null) {
                    ...
                    wsheet.addCell(new DateTime(col, row, DateUtil.stringToDate(o.toString()), wcfFC));
                } else {
                    wsheet.addCell(new Label(col, row, o.toString(), wcfFC));
                }
            }
    ...
    private void setRowView() throws WriteException, IOException {
        try {
            for (int i = 0; i < wsheet.getRows(); i++) {
                wsheet.setRowView(i, (int)(wsheet.getRowView(i).getSize() * 1.3));
            }
        } catch (Exception e) {
            logger.error("", e);
            this.close();
    ...
    private void setColumnView() {
        ...
        for (int i = 0; i < wsheet.getRows(); i++) {
            lablea: for (int j = 0; j < wsheet.getColumns(); j++) {
                ...
                } else if (DataUtil.isNumber(value)) {// 数字
                ...
                cellWidth = wsheet.getColumnView(j).getSize();
