...
public final class InstanceUtil {
    ...
    private final static Map<String, MethodAccess> methodMap = InstanceUtil.newHashMap();
    private final static Map<String, Field> fieldMap = InstanceUtil.newHashMap();
            ...
            for (PropertyDescriptor property : propertyDescriptors) {
                try {
                    ...
                    if (map.containsKey(key)) {
                        Object value = TypeParseUtil.convert(map.get(key), property.getPropertyType(), null);
                        try {
                            String fieldName = clazz.getName() + "." + key;
                            Field field = fieldMap.get(fieldName);
                            if (field == null) {
                    ...
                    logger.error("transMap2Bean setter Error ", e);
                }
            }
        } catch (Exception e) {
                ...
                for (PropertyDescriptor property : propertyDescriptors) {
                    ...
                    if (!"class".equals(key)) {
                        ...
                        if (newValue != null && !newValue.equals(oldValue)) {
                            Object value = TypeParseUtil.convert(newValue, property.getPropertyType(), null);
                            try {
